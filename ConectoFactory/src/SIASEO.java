 ...
 */
public class SIASEO extends AbstractConnector {
	 ...
	 *            The Log to use.
	 */
	public SIASEO(ConnectorConfig config, openconnector.Log log)
			throws Exception {
		super(config, log);
		m_bIsDriverLoaded = false;
		it = null;
		...
		try {
			...
			if (usuarioExiste == false) {
				...
				try {
					...
					if (isDebug == true) {
						log.debug("Entrei no for usuario");
					}
					criarUsuarioOracle(codUsuario, passwordLimpo);

				} catch (ClassNotFoundException e) {
					log.debug("Catch NotFoundException");
					result = new Result(Result.Status.Failed);
					result.add("Erro SIASEO Metodo Create " + e);
					return result;

				} catch (SQLException e) {
					log.debug("Catch SQLException");
					result = new Result(Result.Status.Failed);
					result.add("Erro SIASEO Metodo Create " + e);
					return result;
				}
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo Create " + e);
			result.add("Dados para criacao CO_SISTEMA: " + codSistema + " CO_USUARIO: "
					+ codUsuario + " NUM_PERFIL: " + numPerfil + " PASSWORD: "
					+ passwordLimpo);
			if (log.isDebugEnabled())
	...
	public Result update(String nativeIdentifier, List<Item> items)
		...
		String codSistema = "", codUsuario = "", nomUsuario = "", icStatu = "", icType = "", codSenha = "", numPerfil = "", passwordLimpo = "Partner2015";
		int operacao = 0;
		ArrayList<String> arrayPerfilSistema = new ArrayList<String>();
				...
				if (valueList.size() > 1) {
					for (Object sistemaPerfil : valueList) {
						...
						for (Object sistemaPerfil2 : valueList) {
							...
							if (sistema.equals(sistema2)) {
								result = new Result(Result.Status.Failed);
								result.add("Erro SIASEO metodo Update Operacao ADD "
										+ "O sistema não permite adicionar mais de um perfil.");
								return result;
							}
						}
				...
				case Add: {
					...
					for (String sistemaPerfis : arrayPerfilSistema) {
						...
						try {
							...
							createConnection(codSistema, codUsuario, numPerfil);
							if (isDebug == true) {
								log.debug("UPDADE - PROCEDURE ADD "
										+ "CodUsuario " + codUsuario
										+ " CodSistema " + codSistema
										+ " nomUsuario " + nomUsuario
										+ " icStatu " + icStatu + " icType "
							...
							result = new Result(Result.Status.Failed);
							result.add("Erro SIASEO Metodo Update Operacao ADD " + e);
							return result;
						}
					}
				}
					...
					if (name.equalsIgnoreCase(sisPerfil)) {
						...
						try {
							...
							for (String sistemaPerfis : arrayPerfilSistema) {
								...
								deletarConnectionUsuario(codSistema,
										codUsuario, numPerfil);
							...
							e.printStackTrace();
							result = new Result(Result.Status.Failed);
							result.add("Erro SIASEO Metodo Update Operacao Remove " + e);
							return result;
						}
					}
	...
	public Result delete(String nativeIdentifier, Map<String, Object> options)
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo Delete  " + e1.getMessage());
			return result;
		}
...
//		}
	...
	public Result enable(String nativeIdentifier, Map<String, Object> options)
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo Enable  " + e);
			return result;
		}
...
//		}
	...
	public Result disable(String nativeIdentifier, Map<String, Object> options)
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo Disable   " + e);
			return result;
		}
...
//			log.debug("NEW PASSWORD " + clearPassword);
		...
		try {
			...
			e.printStackTrace();
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo SetPassword  " + e);
			return result;
		}
...
//		}
	...
	public Iterator<Map<String, Object>> getAccounts(String filter)
		...
		try {
			...
			while (queryResult.next()) {
				...
				account.put(noUsuario, queryResult.getString("NO_USUARIO"));
				account.put(icStatus, queryResult.getString("IC_STATUS"));
				account.put(icTipo, queryResult.getString("IC_TIPO"));
				...
				if (isDebug == true) {
					...
					log.debug("icStatus " + queryResult.getString("IC_STATUS"));
					log.debug("icTipo " + queryResult.getString("IC_TIPO"));
			...
			throw new ConnectorException("Erro SIASEO Metodo GetAccount "+  e);
		}
	...
	public Iterator<Map<String, Object>> getGroups(String filter)
		...
		try {
			...
			while (queryResult.next()) {
				...
				String numeroPerfil = queryResult.getString("NU_PERFIL");
			...
			throw new ConnectorException("Erro SIASEO Metodo GetGroup "+  e);
		}
	...
	public void listarUsusario(String codUsuario) {
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: listarUsuario: Procedure: ASESP033_LISTA_USUARIOS " + e);
		} catch (SQLException e) {
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: listarUsuario: Procedure: ASESP033_LISTA_USUARIOS " + e);
	...
	public boolean usuarioExiste(String codUsuario) {
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: usuarioExiste " + e);
		}
	...
	public void listarGroups(String codSitema, String CodPerfil) {
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: listarGroups: procedure: ASESP036_LISTA_PERFIS " + e);
	...
	public void listarConexoeUsuario(String codUsuario) {
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: listarConexoeUsuario: procedure: ASESP037_LISTA_CONEXOES " + e);
		} catch (SQLException e) {
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: listarConexoeUsuario: procedure: ASESP037_LISTA_CONEXOES " + e);
		} catch (Exception e) {
	...
	public void dadosPerfil(String codSitema, String CodPerfil) {
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: dadosPerfil: procedure: ASESP036_LISTA_PERFIS " + e);
		} catch (SQLException e) {
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: dadosPerfil: procedure: ASESP036_LISTA_PERFIS " + e);
		} catch (Exception e) {
	...
	private void criarUsuarioOracle(String coUsuario, String Senha)
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: criarUsuarioOracle: procedure: ASESP906_CREATE_USER " + e);
		}
	...
	public void criarUsuarioTabelasOracle(String codUsuario)
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: criarUsuarioTabelasOracle: procedure: ASESP025_MAN_USUARIO " + e);
		}
	}
	...
	public void createConnection(String coSistema, String coUsuario,
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: createConnection: procedure: ASESP026_MAN_SIS_USUARIO " + e);
		} catch (SQLException e) {
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: createConnection: procedure: ASESP026_MAN_SIS_USUARIO " + e);
		} catch (Exception e) {
	...
	public void deletarConnectionUsuario(String coSistema, String coUsuario,
			String nuPerfil) {
		Result result = new Result();
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: deletarConnectionUsuario: procedure: ASESP026_MAN_SIS_USUARIO " + e);
		} catch (SQLException e) {
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: deletarConnectionUsuario: procedure: ASESP026_MAN_SIS_USUARIO " + e);
		} catch (Exception e) {
	...
	public void listaConexoesDelete(String coUsuario) {
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: listaConexoesDelete: procedure: ASESP037_LISTA_CONEXOES " + e);
		} catch (SQLException e) {
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: listaConexoesDelete: procedure: ASESP037_LISTA_CONEXOES " + e);
		} catch (Exception e) {
	...
	private void deletarUsuarioTabelasOracle(String coUsuario) {
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: deletarUsuarioTabelasOracle: procedure: ASESP025_MAN_USUARIO " + e);
		} catch (SQLException e) {
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: deletarUsuarioTabelasOracle: procedure: ASESP025_MAN_USUARIO " + e);
		} catch (Exception e) {
	...
	private void deletarUsuarioOracle(String coUsuario) {
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: deletarUsuarioOracle: procedure: ASESP910_DROP_USER " + e);
		} catch (SQLException e) {
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: deletarUsuarioOracle: procedure: ASESP910_DROP_USER " + e);
		} catch (Exception e) {
	...
	public void alterarUsuarioTabelasOracle(String coUsuario, String icStatus) {
		Result result = new Result();
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: alterarUsuarioTabelasOracle: procedure: ASESP025_MAN_USUARIO " + e);
		} catch (SQLException e) {
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: alterarUsuarioTabelasOracle: procedure: ASESP025_MAN_USUARIO " + e);
		} catch (Exception e) {
	...
	public void alterarStatus(String coUsuario, String status) {
		Result result = new Result();

		try {
			init();
			cs = callProcedure("{call ASE.ASESP913_MANUT_USER(?, ?) }");
			cs.setString(1, coUsuario);
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: alterarStatus: procedure: ASESP913_MANUT_USER " + e);
		} catch (SQLException e) {
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: alterarStatus: procedure: ASESP913_MANUT_USER " + e);
		} catch (Exception e) {
	...
	private void alterarSenha(String coUsuario, String senhaLimpa) {
		...
		try {
			...
			result = new Result(Result.Status.Failed);
			result.add("Erro SIASEO Metodo: alterarSenha: procedure: ASESP911_PASSWORD " + e);
