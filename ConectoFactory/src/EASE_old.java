 ...
 */
public class EASE_old extends AbstractConnector {
	 ...
	 *            The Log to use.
	 */
	public EASE_old(ConnectorConfig config, openconnector.Log log) throws Exception {
		super(config, log);
		m_bIsDriverLoaded = false;
		it = null;
	}
...
//		ArrayList<String> arrayPerfilSistema = new ArrayList<String>();
		...
		try {
			...
			for (openconnector.Item item : items) {
				...
				if (name.equalsIgnoreCase(nuPerfil)) {
					...
					if (valueList.size() > 1) {
						...
						result.add("TIPO APP EASE "+"O sistema não permite adicionar mais de um perfil.");
			...
			if (!wsRetorno.getErro()) {
				result = new Result(Result.Status.Committed);
			} else {
				result = new Result(Result.Status.Failed);
				result.add("TIPO APP EASE "+wsRetorno.getMensagem());
			}
	...
	public void init() throws Exception {
		...
		if (!m_bIsDriverLoaded) {
			m_sUser = config.getString("login");
			m_sPasswd = config.getString("password");
			m_sUrl = config.getString("wsdl");
